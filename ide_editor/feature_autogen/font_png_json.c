...
void font_png_json_state_init(font_png_json_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "FontPngJson ready");
}

bool font_png_json_validate(const font_png_json_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "FontPngJson: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool font_png_json_handle_command(font_png_json_config *cfg, font_png_json_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "font_png_json.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "FontPngJson enabled");
        return true;
    }
    if (strcmp(command_id, "font_png_json.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "FontPngJson disabled");
        return true;
    }
    return false;
}
...
void font_png_json_tick(font_png_json_config *cfg, font_png_json_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "FontPngJson tick");
}
...
bool font_png_json_deserialize_config(font_png_json_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    font_png_json_config_defaults(cfg);
    return true;
}