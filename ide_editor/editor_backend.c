...
void editor_backend_set_loading_screen(EditorBackend *backend, bool active, const char *title, const char *subtitle, int progress_percent) {
    if (!backend || !backend->ops.set_loading_screen) return;
    backend->ops.set_loading_screen(backend->state, active, title, subtitle, progress_percent);
}
