...
void engine_source_extension_state_init(engine_source_extension_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "EngineSourceExtension ready");
}

bool engine_source_extension_validate(const engine_source_extension_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "EngineSourceExtension: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool engine_source_extension_handle_command(engine_source_extension_config *cfg, engine_source_extension_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "engine_source_extension.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "EngineSourceExtension enabled");
        return true;
    }
    if (strcmp(command_id, "engine_source_extension.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "EngineSourceExtension disabled");
        return true;
    }
    return false;
}
...
void engine_source_extension_tick(engine_source_extension_config *cfg, engine_source_extension_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "EngineSourceExtension tick");
}
...
bool engine_source_extension_deserialize_config(engine_source_extension_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    engine_source_extension_config_defaults(cfg);
    return true;
}