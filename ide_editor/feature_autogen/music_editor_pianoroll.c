...
void music_editor_pianoroll_state_init(music_editor_pianoroll_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "MusicEditorPianoroll ready");
}

bool music_editor_pianoroll_validate(const music_editor_pianoroll_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "MusicEditorPianoroll: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool music_editor_pianoroll_handle_command(music_editor_pianoroll_config *cfg, music_editor_pianoroll_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "music_editor_pianoroll.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "MusicEditorPianoroll enabled");
        return true;
    }
    if (strcmp(command_id, "music_editor_pianoroll.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "MusicEditorPianoroll disabled");
        return true;
    }
    return false;
}
...
void music_editor_pianoroll_tick(music_editor_pianoroll_config *cfg, music_editor_pianoroll_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "MusicEditorPianoroll tick");
}
...
bool music_editor_pianoroll_deserialize_config(music_editor_pianoroll_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    music_editor_pianoroll_config_defaults(cfg);
    return true;
}