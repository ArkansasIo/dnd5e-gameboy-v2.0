...
void platformer_advanced_moves_state_init(platformer_advanced_moves_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "PlatformerAdvancedMoves ready");
}

bool platformer_advanced_moves_validate(const platformer_advanced_moves_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "PlatformerAdvancedMoves: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool platformer_advanced_moves_handle_command(platformer_advanced_moves_config *cfg, platformer_advanced_moves_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "platformer_advanced_moves.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "PlatformerAdvancedMoves enabled");
        return true;
    }
    if (strcmp(command_id, "platformer_advanced_moves.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "PlatformerAdvancedMoves disabled");
        return true;
    }
    return false;
}
...
void platformer_advanced_moves_tick(platformer_advanced_moves_config *cfg, platformer_advanced_moves_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "PlatformerAdvancedMoves tick");
}
...
bool platformer_advanced_moves_deserialize_config(platformer_advanced_moves_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    platformer_advanced_moves_config_defaults(cfg);
    return true;
}