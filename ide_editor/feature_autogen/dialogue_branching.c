...
void dialogue_branching_state_init(dialogue_branching_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "DialogueBranching ready");
}

bool dialogue_branching_validate(const dialogue_branching_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "DialogueBranching: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool dialogue_branching_handle_command(dialogue_branching_config *cfg, dialogue_branching_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "dialogue_branching.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "DialogueBranching enabled");
        return true;
    }
    if (strcmp(command_id, "dialogue_branching.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "DialogueBranching disabled");
        return true;
    }
    return false;
}
...
void dialogue_branching_tick(dialogue_branching_config *cfg, dialogue_branching_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "DialogueBranching tick");
}
...
bool dialogue_branching_deserialize_config(dialogue_branching_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    dialogue_branching_config_defaults(cfg);
    return true;
}