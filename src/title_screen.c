...
// --- SRD-Core Splash ---
static void init_srd_core_splash(void) {
  DISPLAY_OFF;
  scroll_bkg(0, 0);
  ...
  // Draw "SRD-Core Engine" text in the center (simple text for now)
  core.draw_text((uint8_t*)"SRD-Core Engine", 60, 80);
  DISPLAY_ON;
}

void init_title_screen(void) BANKED {
  init_srd_core_splash();
  title_state = TITLE_SRD_CORE_SPLASH;
}

void update_title_screen(void) BANKED {
  static uint8_t srd_timer = 0;
  switch (title_state) {
  case TITLE_SRD_CORE_SPLASH:
    // Wait ~1.5 seconds, then move to next splash
    if (++srd_timer > 90) {
...
void init_main_title(void) {
  ...
  core.draw_text((uint8_t*)"Dungeons & Dragons:", 24, 32);
  core.draw_text((uint8_t*)"The Black Grimoire", 32, 48);
