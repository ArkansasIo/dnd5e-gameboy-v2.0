...
void camera_scrolling_state_init(camera_scrolling_state *state) {
    ...
    safe_copy(state->last_status, (int)sizeof(state->last_status), "CameraScrolling ready");
}

bool camera_scrolling_validate(const camera_scrolling_config *cfg, char *reason, int reason_len) {
    if (reason && reason_len > 0) {
        snprintf(reason, (size_t)reason_len, "CameraScrolling: validation ok");
    }
    if (!cfg) return false;
    return cfg->enabled;
}
...
bool camera_scrolling_handle_command(camera_scrolling_config *cfg, camera_scrolling_state *state, const char *command_id) {
    ...
    if (strcmp(command_id, "camera_scrolling.enable") == 0) {
        cfg->enabled = true;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "CameraScrolling enabled");
        return true;
    }
    if (strcmp(command_id, "camera_scrolling.disable") == 0) {
        cfg->enabled = false;
        safe_copy(state->last_status, (int)sizeof(state->last_status), "CameraScrolling disabled");
        return true;
    }
    return false;
}
...
void camera_scrolling_tick(camera_scrolling_config *cfg, camera_scrolling_state *state) {
    ...
    state->frame_counter++;
    cfg->revision++;
    safe_copy(state->last_status, (int)sizeof(state->last_status), "CameraScrolling tick");
}
...
bool camera_scrolling_deserialize_config(camera_scrolling_config *cfg, const char *path) {
    (void)path;
    if (!cfg) return false;
    camera_scrolling_config_defaults(cfg);
    return true;
}